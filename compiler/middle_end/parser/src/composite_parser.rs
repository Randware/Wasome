...
use crate::misc_parsers::{
    datatype_parser, identifier_parser, map_visibility, maybe_statement_separator,
    statement_separator, token_parser, visibility_parser,
};
...
-> impl Parser<'src, &'src [PosInfoWrapper<TokenType>], ASTNode<Struct<UntypedAST>>> {
    ...
    visibility_parser()
        ...
        .map(
            |(visibility, ((((struct_token, name), fields), functions), end))| {
                let fields = fields
                    .into_iter()
