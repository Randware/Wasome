...
impl<'ctx> Codegen<'ctx> {
    pub(crate) fn compile_inc_refcount(
        ...
        let refc = llvm_context.builder().build_struct_gep(llvm_context.global_registry().base_heap_allocated(), to_generate, 0, "gep_refc_inc").unwrap();
        let refc_val = llvm_context.builder().build_load(self.context.i32_type(), refc, "load_refc_inc").unwrap();
        let inc = llvm_context.builder().build_int_add(refc_val.into_int_value(), self.context.i32_type().const_int(1, false), "inc_refc").unwrap();
    ...
    pub(crate) fn compile_struct_dec_refcount(
        ...
        let refc = llvm_context.builder().build_struct_gep(llvm_context.global_registry().base_heap_allocated(), to_generate, 0, "gep_refc_dec").unwrap();
        let refc_val = llvm_context.builder().build_load(self.context.i32_type(), refc, "load_refc_dec").unwrap();
        let dec = llvm_context.builder().build_int_sub(refc_val.into_int_value(), self.context.i32_type().const_int(1, false), "dec_refc").unwrap();
        ...
        llvm_context.builder().build_call(lowered.on_drop(), &[to_generate.as_basic_value_enum().into()], "drop_struct").unwrap();
        llvm_context.builder().build_unconditional_branch(after_bb).unwrap();
    ...
    pub(crate) fn compile_struct_drop(
        ...
        let after_block = llvm_context.context().append_basic_block(func, "after_block");

        match lowered.predrop() {
            None => {
                llvm_context.builder().build_unconditional_branch(after_block).unwrap();
            },
            Some(predrop) => {
                let drop_abort_block = llvm_context.context().append_basic_block(func, "drop_abort_block");
                ...
                self.compile_inc_refcount(llvm_context, to_generate);
                llvm_context.builder().build_call(predrop, &[to_generate.as_basic_value_enum().into()], "predrop").unwrap();
        ...
        for (i, field) in lowered.fields().iter().enumerate() {
            match field.data_type() {
                DataType::Struct(st) => {
                    let field = llvm_context.builder().build_struct_gep(lowered.lowered(), to_generate, (i+1) as u32, "gep_field_drop").unwrap();
    ...
    pub(crate) fn compile_enum_drop(
        ...
        let cond_blocks = lowered.variants().iter().map(|variant| {
            llvm_context.context().append_basic_block(func, "cond")
        }).collect::<Vec<_>>();
        let drop_blocks = lowered.variants().iter().map(|variant| {
            llvm_context.context().append_basic_block(func, "drop")
        }).collect::<Vec<_>>();
        let tag = llvm_context
            .builder()
            .build_struct_gep(llvm_context.global_registry().base_enum(), to_generate, 1, "tag_gep")
            .expect("Enum must have tag");
        let tag = llvm_context
            .builder()
            .build_load(self.context.i32_type(), tag, "load_load")
            .unwrap();
        for (i, (variant, (cond, drop))) in lowered.variants().iter().zip(cond_blocks.iter().zip(drop_blocks.iter())).enumerate() {
            ...
            for (i, field) in variant.0.fields().iter().enumerate() {
                match field {
                    DataType::Struct(st) => {
                        let field = llvm_context.builder().build_struct_gep(variant.1, to_generate, (i+2) as u32, "gep_field_drop").unwrap();
                        self.compile_struct_dec_refcount(llvm_context, func, st, field)
                    }
                    DataType::Enum(en) => {
                        let field = llvm_context.builder().build_struct_gep(variant.1, to_generate, (i+2) as u32, "gep_field_drop").unwrap();
